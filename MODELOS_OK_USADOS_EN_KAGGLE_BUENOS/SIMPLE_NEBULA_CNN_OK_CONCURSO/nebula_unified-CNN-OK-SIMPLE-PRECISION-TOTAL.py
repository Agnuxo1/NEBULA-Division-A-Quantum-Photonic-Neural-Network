...
def main():
    ...
    if Path('optimized_thresholds.npy').exists() and threshold_optimizer.best_thresholds is not None:
        logger.info("🎯 Applying optimized thresholds to test predictions...")
        optimized_thresholds = np.load('optimized_thresholds.npy')
        
        # Convert probabilities to binary using optimized thresholds
        binary_predictions = (predictions >= optimized_thresholds).astype(int)
        
        logger.info("✅ Test predictions enhanced with optimized thresholds!")
    else:
        logger.info("⚠️  Using default 0.5 thresholds for test predictions")
    ...
    # Use binary predictions with optimized thresholds
    for i, col in enumerate(condition_columns):
        submission[col] = binary_predictions[:, i]
    
    # Save submission
    submission_path = 'nebula_submission.csv'
