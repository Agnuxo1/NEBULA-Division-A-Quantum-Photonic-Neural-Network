"""
NEBULA for Grand X-Ray Slam Division A - v3.0 ENHANCED
=========================================================
ENHANCED VERSION with Auto-Vision Calibration Integration
Base: v1.0 Hybrid (Ray-tracing + CNN) - WORKING ARCHITECTURE
                    ...
                    if i + 1 < max_quantum_idx:
                        ...
                        with torch.no_grad():
                            ...
                            if mask.shape[0] != target_shape[0]:  # Batch dimension mismatch
                                # Take first sample for expansion template, then broadcast back
                                mask = mask[0].expand(target_shape)
                            else:
...
class RSNAMasterTrainer:
    ...
    def _run_classification(self, train_df, val_df, resume_checkpoint=None):
        ...
        for epoch in range(start_epoch, self.config.classification_epochs + 1):
            ...
            for batch in pbar:
                ...
                # CRITICAL FIX: Handle dict outputs (same as SOLUCION_ERROR_DIMENSIONAL.md)
                if isinstance(outputs, dict):
                    ...
                    if 'loss' in outputs:
                        ...
                        loss = criterion(classification_tensor, batch['targets'].to(DEVICE))
                else:
    ...
    def _validate_epoch(self, epoch, model, loader, criterion):
        ...
        with torch.no_grad():
            for batch in tqdm(loader, desc=f"Epoch {epoch} [Val]"):
                ...
                # Handle dict outputs for validation (same pattern as training)
                if isinstance(outputs, dict):
                    classification_tensor = outputs.get('classification', 
                                                       outputs.get('logits', 
                                                       outputs.get('pred', 
    ...
    def _get_dataloaders(self, train_df, val_df):
        # Use GrandXRayDataset (not RSNADataset) for Grand X-Ray Slam
        train_dataset = GrandXRayDataset(self.config.train_images_dir, train_df, self.config.resolution_2d)
        val_dataset = GrandXRayDataset(self.config.train_images_dir, val_df, self.config.resolution_2d)
