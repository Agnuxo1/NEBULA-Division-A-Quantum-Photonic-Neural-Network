...
class CalibratedStepAnalyzer(nn.Module):
    ...
    def forward(self, xray_image: torch.Tensor) -> torch.Tensor:
        ...
        for i, wavelength in enumerate(self.wavelengths):
            ...
            # Use calibrated parameters as-is - don't modify them
            absorption = float(absorption)
            scattering = float(scattering) 
            penetration = float(penetration)
            
            # Wavelength-specific tissue interaction (calibrated) - stabilized
            tissue_response = phantom * torch.exp(-torch.clamp(phantom * absorption, 0, 5.0))  # Absorption
            tissue_response *= torch.exp(-torch.clamp(phantom * scattering, 0, 5.0))  # Scattering - stabilized
            tissue_response *= (1 + penetration)  # Penetration - already clamped
            
            # CRITICAL: Check for NaN in tissue response
            if torch.isnan(tissue_response).any() or torch.isinf(tissue_response).any():
                tissue_response = torch.where(torch.isnan(tissue_response) | torch.isinf(tissue_response),
                                            torch.ones_like(tissue_response) * 0.5, tissue_response)
            
            # Apply calibrated laser intensity and sensor sensitivity
            intensity = max(0.1, min(5.0, float(self.laser_intensities[i])))
            sensitivity = max(0.1, min(5.0, float(self.sensor_sensitivities[i])))
            
            final_response = tissue_response * intensity * sensitivity
            
            # CRITICAL: Final response NaN check
            if torch.isnan(final_response).any() or torch.isinf(final_response).any():
                final_response = torch.where(torch.isnan(final_response) | torch.isinf(final_response),
                                           torch.ones_like(final_response) * 0.5, final_response)
            
            # Clamp final response to reasonable range
            final_response = torch.clamp(final_response, 0.0, 10.0)
